...
import { create, lookup } from '@remusao/trie';
import fs from 'fs';
...
let data = fs.readFileSync('./src/util/dict.txt', 'utf8').split('\n');
...
const populateSet = (props: Props) => {
  ...
  const permuteWords = (wordBank, currWord, wordList, trie) => {
    if (lookup(trie, currWord)) {
      wordList.add(currWord);
    }

    for (const letter of wordBank) {
      permuteWords(wordBank.replace(letter, ''), currWord + letter, wordList, trie);
    }
  }
